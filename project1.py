# Part I: Functions and function templates that we used to implement our learning algorithms.
...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    # Code here
    agreement = np.matmul(theta,feature_vector.T) + theta_0
    hypothesis = label * agreement
    if hypothesis < 1:
        return(1 - hypothesis)
    else:
        return (0)
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    # Code here
    agreement = np.dot(theta,feature_matrix.T) + theta_0
    hypothesis = labels*agreement
    sumx = 0
    for i in hypothesis:
        if i < 1:
            sumx += 1 - i
            ...
            sumx += 0
    return sumx/hypothesis.shape[0]
#pragma: coderesponse end
...
def pegasos_single_step_update(
    ...
    """
    # Code here
    agreement = np.dot(current_theta,feature_vector.T) + current_theta_0
    hypothesis = label*agreement
    if hypothesis <= 1:
...
def pegasos(feature_matrix, labels, T, L):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            ...
            c += 1
            current_theta,current_theta_0 = \
            pegasos_single_step_update(feature_matrix[i,:],labels[i],L,eta,current_theta,current_theta_0)
...
def classifier_accuracy(
    ...
    # Using these parameters to get predictions for training and validation sets
    pred_train = classify(train_feature_matrix, theta, theta_0)
    pred_val = classify(val_feature_matrix, theta, theta_0)
    
    # Calculating classification accuracy by comparing predictions with labels
